...
//per ottenere l'inode del file

unsigned long get_inode(const char *file_path) {
    ...
    return (uint64_t)fileStat.st_ino;
}
    ...
    while (fgets(line, sizeof(line), file)) {
        ...
        if (strcmp(current_type, type) == 0) {
            ...
            while (token != NULL) {
                ...
                printf("DEBUG => inode: %lu; \n", inode);
                ret = bpf_map__update_elem(map_file,&inode,sizeof(inode),&ok_value,sizeof(ok_value),BPF_ANY);
                if (ret < 0) {
...
int main(int argc, char **argv)
{
	...
	skel = LSMopen_bpf__open_and_load();
	if (!skel) {
    ...
    map_file = skel->maps.OPEN_FILES_INODE_MAP;
    if (map_file < 0) {
