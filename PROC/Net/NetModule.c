...
int main(int argc, char **argv)
{
    ...
    struct PROCNetModule_bpf *skel;
    struct bpf_map *map_proc;
	int err;

	
	/* Open load and verify BPF application */
	skel = PROCNetModule_bpf__open_and_load();
	if (!skel) {
	...
	err = PROCNetModule_bpf__attach(skel);
	if (err) {
...
cleanup:
	PROCNetModule_bpf__destroy(skel);
	return -err;
